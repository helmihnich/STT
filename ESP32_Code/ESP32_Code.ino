...
// WIFI Configuration
const char* ssid     = "Infinix";  //configuration wifi nom et mot de passe
...
void setup() {
 ...
 // pinMode (Buzzer, OUTPUT);
  ...
  display.setTextColor(BLUE);tempsensor3.begin(0x1A);
...
void loop() {
  ...
  Serial.print("Valeur de T1 : ");
  Serial.println(T1);
  Serial.print("Valeur de T2 : ");
  Serial.println(T2);
  Serial.print("Valeur de T3 : ");
  Serial.println(T3);
  ...
  Serial.print("La valeur moyenne de T3 : ");
  Serial.println(T_M);
...
void send_sensor_value(float sensor_data_1, float sensor_data_2, float sensor_data_3, float tempmoy){
  ...
  http.begin("http://192.168.1.20:3000/fridge_1");
  // Define json type 
  http.addHeader("Content-Type", "application/json");

  // Data that we want to send 
  String request = "{\"sensor_value1_1\":" +(String)(1.2)+ ", \"sensor_value1_2\":"+(String)(5.1)+", \"sensor_value1_3\":"+(String)(4.8)+", \"temperature_moyen1\":"+(String)(8)+", \"carte\":"+"true"+ "}";
...
void buzzer(){
  HTTPClient http;
      http.begin("http://192.168.1.20:3000/buzz_esp1");
...
void etat_buzz (){
  ...
  HTTPClient http;
      http.begin("http://192.168.1.20:3000/buzz1");
    String request = "55";
   int http_response_code = http.POST(request);
